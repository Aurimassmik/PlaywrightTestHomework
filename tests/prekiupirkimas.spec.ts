...
test("Success Buying muffin", async ({ page }) => {
  ...
  await page
    .locator(
      'h6[data-qa="product-list-section-item-title"]:has-text("Glazed Paradise Donuts")'
    )
    .click();
  ...
  //Select Lithuania from country list  !!!!need time to iterate code
  await page.locator('[data-qa="checkout-shippingdestination-select"]').click();
  await page.getByRole("option", { name: "Lithuania" }).click();
  //select shipping from list
  await page
    .locator('[data-qa="checkout-shippingoptions-parcelselect"]')
    .click();
  await page
    .getByRole("option", {
      name: "Akmenės NORFA Daukanto paštomatas, S. Daukanto g. 7",
    })
    .click();
  //Confirm shiping and country choice
  ...
  await page
    .getByRole("textbox", { name: "Your phone number" })
    .fill("+37062545874");
  await page
    .getByRole("textbox", { name: "Your comment" })
    .fill("shipping was hard");
  await page
    .locator('[data-qa="checkout-contactinformation-continue"]')
    .click();
  //confirm buying
  await page.getByTestId("checkout-paymentmethods-placeorder").click();
  //Check for popup
  await expect(page.getByText("Your order has been received.")).toHaveClass(
    "payment-info__text body-large"
  );
  //Close popup
